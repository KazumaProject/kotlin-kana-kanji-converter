...
| 区分 | 件数 | 実行時の扱い |
| --- | ---: | --- |
| `primary` | 92,859 | 自然な英単語・英語句として通常順位 |
| `review` | 24,690 | 監査表だけに保持し、実行時辞書から除外 |
| `excluded` | 1,162 | 不自然な読み・記号・数値表記・単独機能語・間投詞・接辞・未完の省略などを通常辞書から除外 |
| 実行時の採用件数 | 92,859 | primary のみ。正規化・重複除去後 |
| 実行時に検索できる読み | 59,687 | ノイズ除去済み辞書のキー |
