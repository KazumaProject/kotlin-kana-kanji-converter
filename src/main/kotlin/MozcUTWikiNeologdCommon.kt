...
private fun buildDictionaryForNeologd() {
    println("start build Wiki Neologd Common dictionary")
    ...
    val dictionaryList = readingCorrectionBuilder.parseMozcUTDictionaryCompressedDictionary(
        readTextFromZip(
            filePath = "src/main/bin/wiki_neologd_common.txt.zip",
            fileName = "wiki_neologd_common.txt"
        )
    )
        .groupBy { it.yomi }
        .toSortedMap(compareBy({ it.length }, { it }))
    ...
    val objectOutputYomi =
        ObjectOutputStream(BufferedOutputStream(FileOutputStream("./src/main/resources/yomi_web.dat")))
    val objectOutputTango =
        ObjectOutputStream(BufferedOutputStream(FileOutputStream("./src/main/resources/tango_web.dat")))

    yomiLOUDSTemp.writeExternalNotCompress(objectOutputYomi)
    tangoLOUDSTemp.writeExternalNotCompress(objectOutputTango)

    val objectInputYomi = ObjectInputStream(FileInputStream("./src/main/resources/yomi_web.dat"))
    val objectInputTango = ObjectInputStream(FileInputStream("./src/main/resources/tango_web.dat"))
    ...
    val objectOutput = ObjectOutputStream(FileOutputStream("./src/main/resources/token_web.dat"))
    tokenArrayTemp.buildTokenArray(dictionaryList, tangoLOUDS, objectOutput, 1)
    val objectInput = ObjectInputStream(FileInputStream("./src/main/resources/token_web.dat"))
    val tokenArray = TokenArray()
